...
export class Cart extends Component<ICartView> {
	...
	renderCart() {
		...
		this._cartItems.forEach((item, index) => {
			...
			const basketItemContent = basketItemTemplate.content.cloneNode(
				true
			) as HTMLElement;

			const indexElement = basketItemContent.querySelector(
				'.basket__item-index'
			) as HTMLElement;
			const titleElement = basketItemContent.querySelector(
				'.card__title'
			) as HTMLElement;
			const priceElement = basketItemContent.querySelector(
				'.card__price'
			) as HTMLElement;
			const deleteButton = basketItemContent.querySelector(
				'.basket__item-delete'
